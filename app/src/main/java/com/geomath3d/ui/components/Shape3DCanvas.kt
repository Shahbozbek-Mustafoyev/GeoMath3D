    ...
    Canvas(
        ...
        when (shapeType) {
            ...
            ShapeType.CYLINDER -> {
                ...
                val steps = 36
                // Bottom face
                val bottomPts = (0..steps).map { i ->
                    val a = i * 2 * PI.toFloat() / steps
                    pt(r * cos(a), h, r * sin(a))
                }
                // Top face
                val topPts = (0..steps).map { i ->
                    val a = i * 2 * PI.toFloat() / steps
                    pt(r * cos(a), -h, r * sin(a))
                }
                // Side fill (visible half)
                val sidePath = Path().apply {
                    moveTo(topPts[0].x, topPts[0].y)
                    for (i in 0..steps / 2) lineTo(topPts[i].x, topPts[i].y)
                    for (i in steps / 2 downTo 0) lineTo(bottomPts[i].x, bottomPts[i].y)
                    close()
                }
                drawPath(sidePath, fill)
                // Bottom ellipse
                val bottomPath = Path().apply {
                    moveTo(bottomPts[0].x, bottomPts[0].y)
                    bottomPts.forEach { lineTo(it.x, it.y) }
                    close()
                }
                drawPath(bottomPath, Brush.linearGradient(
                    listOf(secondaryColor.copy(.7f), secondaryColor.copy(.4f))
                ))
                // Top ellipse
                val topPath = Path().apply {
                    moveTo(topPts[0].x, topPts[0].y)
                    topPts.forEach { lineTo(it.x, it.y) }
                    close()
                }
                drawPath(topPath, Brush.linearGradient(
                    listOf(primaryColor.copy(.85f), primaryColor.copy(.6f))
                ))
                // Edges
                drawPath(Path().apply {
                    moveTo(topPts[0].x, topPts[0].y)
            ...
            ShapeType.CONE -> {
                ...
                drawPath(basePath, Brush.linearGradient(
                ...
                drawLine(edgeColor, apex, basePts[0], strokeWidth = 1.5f)
                drawLine(edgeColor, apex, basePts[steps / 2], strokeWidth = 1.5f)
            }
